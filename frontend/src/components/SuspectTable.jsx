...
import SuspectEditModal from "./SuspectEditModal";
...
const SuspectTable = () => {
  ...
  const remove = async (id) => {
    if (window.confirm("Delete this suspect?")) {
      try {
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 mt-8">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <h3 className="text-2xl font-bold text-gray-800">Suspect Records</h3>
          <button
            onClick={handlePrint}
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className="bg-gray-50">
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Crime Code</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered.map((suspect) => (
                <tr key={suspect._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{suspect.officerInCharge}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{suspect.createdBy}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {new Date(suspect.createdAt).toLocaleString()}
                  </td>
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          ...
          No suspects found matching your search criteria
        </div>
      )}
    </div>
  );
