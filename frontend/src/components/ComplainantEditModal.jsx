...
const ComplainantEditModal = ({ open, onClose, record, onUpdate }) => {
  ...
  const eyeColorOptions = ["Brown", "Black", "Blue", "Green", "Hazel", "Gray"];
  const hairColorOptions = ["Black", "Brown", "Blonde", "Red", "Gray", "White"];

  const validateForm = () => {
    const newErrors = {};
    const requiredFields = ["name", "phone", "address", "state", "lga"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        formData.append('type', 'complainant');
      ...
      await API.put(`/complainants/${record._id}`, form);
      onUpdate();
      onClose();
    } catch (error) {
      console.error("Error updating complainant:", error);
          ...
          <div className="flex-shrink-0">
            <div className="w-32 h-32 border-2 border-gray-300 rounded-lg overflow-hidden bg-gray-50">
              {imagePreview ? (
                ...
                <ImageWithAuth
                  src={`/complainants/photo/${record._id}`}
                  alt="Current photo"
                  className="w-full h-full object-cover"
                />
              ) : (
  ...
  const renderField = (field) => {
    ...
    const isSelect = ["gender", "complexion", "eyeColor", "hairColor"].includes(field);
    const isRequired = ["name", "phone", "address", "state", "lga"].includes(field);

    let options = [];
    if (field === "gender") options = genderOptions;
    else if (field === "complexion") options = complexionOptions;
    else if (field === "eyeColor") options = eyeColorOptions;
    else if (field === "hairColor") options = hairColorOptions;
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {renderField("eyeColor")}
            {renderField("hairColor")}
...
export default ComplainantEditModal;